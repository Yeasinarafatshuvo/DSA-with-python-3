...
def test_append():
    ...
    assert str(dll) == "5,8", "string representation of dll should be 5,8"
...
def test_prepend():
    ...
    assert dll.head.next.data == new_item, "head should point to the new_node"

    new_node = dll.head.next
    old_node = new_node.next
    assert new_node.prev == None, "previous node of new_node should be None cause it's head node"
    assert old_node.prev == new_node, "previous node of the old node should be new_node"
    assert old_node.data == item, "checking the data of the old node"
...
def test_search():
    ...
    item = 5
    assert dll.search(item) == None, "item should be found in an empty list"
    dll.append(item)
...
def test_remove():
    ...
    assert str(dll) == "5,10", "20 should not be list anymore"

    #remove from start
    dll.remove(5)
    assert dll.head.next == node_10, "now head should point to be 10"
